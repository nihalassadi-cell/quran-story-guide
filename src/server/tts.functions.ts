...
// repeat callers always hit the cache. No auth required: input is a bounded
...
const KALIMA_VOICE_ID = "pNInz6obpgDQGcFmaJgB"; // Adam — male reciter voice

export const getKalimaAudio = createServerFn({ method: "POST" })
  .inputValidator((data: { text: string }) => {
    if (typeof data?.text !== "string") throw new Error("invalid_text");
      ...
      const res = await fetch(
        `https://api.elevenlabs.io/v1/text-to-speech/${KALIMA_VOICE_ID}?output_format=mp3_44100_128`,
        {
          ...
          body: JSON.stringify({
            text: data.text,
            model_id: "eleven_multilingual_v2",
            voice_settings: { stability: 0.6, similarity_boost: 0.8, style: 0.2, use_speaker_boost: true, speed: 0.9 },
          }),
        },
      );
      if (!res.ok) {
