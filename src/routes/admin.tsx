  ...
  return (
    <AppShell>
      <div className="max-w-2xl mx-auto px-4 pt-8 pb-24">
        ...
        <p className="text-sm text-muted-foreground mb-5">Generate AI scenes for any Surah. Verses mentioning the Prophet ﷺ get symbolic imagery only.</p>

        {status === "loading" && <p className="text-sm text-muted-foreground">Checking permissions…</p>}
        {status === "denied" && (
          <div className="rounded-xl border border-border bg-card/60 p-6">
            <p className="text-sm">You do not have admin access. An existing admin must grant your account the admin role.</p>
          </div>
        )}
        {status === "ok" && (
